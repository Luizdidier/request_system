...
function Show({ auth, errors, request }) {
  ...
  const requestTotal = request.products.reduce(
    (previousValue, currentValue) => {
      return (
        previousValue +
        Number(currentValue.pivot.unit_value) *
          Number(currentValue.pivot.quantity)
      );
    },
    0
  );
              ...
              <div>
                <div className="flex items-center justify-center ">
                  <div className="w-full bg-white shadow-lg">
                            ...
                            <tbody className="bg-white">
                              {request.products.map((el) => (
                                <tr className="whitespace-nowrap text-center">
                                  ...
                                  <td className="px-6 py-4 text-sm text-gray-500">
                                    <CurrencyFormat
                                      value={el.pivot.unit_value.replace(
                                        '.',
                                        ','
                                      )}
                                      decimalSeparator=","
                    ...
                    <div className="p-4 flex items-end justify-end space-x-3">
                      <ReactToPrint
                        trigger={() => (
                          <button className="inline-flex items-center px-4 py-2 bg-gray-300 border border-transparent rounded-md font-semibold text-xs text-black uppercase tracking-widest active:bg-gray-600 transition ease-in-out duration-150">
                            Imprimir
                          </button>
                        )}
